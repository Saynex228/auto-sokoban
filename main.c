...
int main() {
    ...
    chargerDeplacements(tab, nomFichierDep, &nbDeplacements);
    ...
    while(partieEnCours) {
        ...
        deplacer(tab, plateau, plateau_initial, *nbDeplacements);




    }
    traiter_victoire(nbDeplacements, &win, nomFichier, nomFichierDep )
        if(gagne(plateau)) {
        continue;
    }
...
void deplacer(typeDeplacements tab, t_Plateau plateau,
    ...
    char touche = typeDeplacements[*nbDeplacements];
    int dx = 0;
    int dy = 0;
    switch(touche) {
        case 'q': dx = -1; break;
 ...
 */
void traiter_victoire(int nbDeplacements, bool *win, char nomFichier, char nomFichierDep) {
    if(!win) {
        printf("\nLa suite de déplacements %s N’EST PAS une solution pour\n
            la partie %s.\n", nomFichierDep, nomFichier);
    }
    else { 
        printf("La suite de déplacements %s est bien une solution pour\n
        la partie %s.\n
        Elle contient %d déplacements.\n", nomFichierDep, nomFichier, nbDeplacements);
    }
